...
public class ItemKey implements Comparable<ItemKey>
{
    ...
    {
        if(item.itemID != o.item.itemID) return Integer.valueOf(item.itemID).compareTo(o.item.itemID);
        if(item.getItemDamage() != o.item.getItemDamage()) return Integer.valueOf(item.getItemDamage()).compareTo(o.item.getItemDamage());
        return 0;
    }
}
