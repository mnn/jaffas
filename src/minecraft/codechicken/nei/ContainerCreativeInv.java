...
public class ContainerCreativeInv extends Container
{
    public ContainerCreativeInv(EntityPlayer player, ExtendedCreativeInv extraInv)
    {
        ...
        for(int row = 0; row < 6; row++)
        {
            ...
            {
                this.addSlotToContainer(new Slot(extraInv, col + row * 9, 8 + col * 18, 5 + row * 18));
            }
        }
        ...
        for(int row = 0; row < 3; ++row)
        {
            for(int col = 0; col < 9; ++col)
            {
                this.addSlotToContainer(new Slot(invPlayer, col + row * 9 + 9, 8 + col * 18, 118 + row * 18));
            }
        }

        for(int col = 0; col < 9; ++col)
        {
            this.addSlotToContainer(new Slot(invPlayer, col, 8 + col * 18, 176));
        }
        
        addSlotToContainer(new SlotBlockArmor(null, invPlayer, invPlayer.getSizeInventory() - 1, -15, 23, 0));
