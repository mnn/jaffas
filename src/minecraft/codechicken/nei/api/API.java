...
import codechicken.nei.GuiNEIControls.NEIKeyBind;
...
public class API
{
    ...
    */
    ...
    /**
     * Add a setting to the Ingame Settings menu.
     * WARNING: Make sure that if you are using NEI's config you initialise the default value of the setting with a call to {@link NEIClientConfig}.globalConfig.getTag(tagname).setDefualtValue(value);
     * @param o The option to add.
     */
    public static void addSetting(NEIOption o)
    {
        addSetting(GuiNEISettings.class, o);
    }
    
    /**
     * A bit more indepth, to add options to other settings menus
     * @param guiclass The class of the Setting Gui to add to
     * @param o The option to add.
     */
    public static void addSetting(Class<? extends GuiNEISettings> guiclass, NEIOption o)
    {
        LinkedList<NEIOption> list = GuiNEISettings.optionMap.get(guiclass);
        if(list == null)
        {
            list = new LinkedList<NEIOption>();
    ...
    public static void addKeyBind(String ident, String desc, int defaultKey)
    {
        ...
        addKeyBind(GuiNEIControls.class, new NEIKeyBind(ident, desc));
    }
    
    /**
     * And here we go again, look at the addSetting version, or the function above! You'll know what to do
     */
    public static void addKeyBind(Class<? extends GuiNEIControls> guiclass, NEIKeyBind key)
    {
        LinkedList<NEIKeyBind> list = GuiNEIControls.keyBindMap.get(guiclass);
        if(list == null)
        {
            list = new LinkedList<NEIKeyBind>();
    ...
    {
        ArrayList<IHighlightIdentifier> handlers = ItemInfo.highlightIdentifiers.get(blockID);
        if(handlers == null)
        {
            handlers = new ArrayList<IHighlightIdentifier>();
